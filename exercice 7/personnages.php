                    ...
                    if ($critchance == 0){
                        ...
                        echo "<p>".$this->_pseudo." attaque ".$personnage->getPseudo()." il perd 20 pv, il lui reste ". $personnage->getVie() ." pv</p>";
                            try{
                                $BDD = new PDO('mysql:host=192.168.64.116; dbname=exo_php_thomas;
                                charset=utf8','admin', 'admin');
                                echo 'voici la requete' ;
                                echo 'UPDATE `personnages` SET `vie`= '.$personnage->getVie().' WHERE id_personnage='.$personnage->getID();
                                $vieBDD = $BDD ->query('UPDATE `personnages` SET `vie`= '.$personnage->getVie().' WHERE id_personnages='.$personnage->getID());
                                $infoBDD = $BDD ->query('select * from personnages where id_personnages="'.$personnage->getID().'"');
                            }
